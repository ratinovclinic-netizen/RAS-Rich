    ...
    <section className="rich-main">
      ...
      {metrics && <>
        ...
        {funnel && <section className="rich-panel rich-drill"><div className="drill-head"><div><span>ГЛУБОКИЙ ПРОВАЛ</span><h2>{funnel.name}</h2><p>ЦКП → воронка → стадия → сделка → контакты</p></div><button onClick={() => { setSelectedFunnel(null); setDealDetail(null); }}>Закрыть</button></div>
          ...
          {!drillLoading && selectedStage && <div className="deal-grid"><div className="deal-list"><h3>Сделки стадии</h3>{deals.map((deal) => <button key={deal.id} onClick={() => void openDeal(deal.id)}><div><strong>{deal.title || `Сделка №${deal.id}`}</strong><span>Обновлено {deal.updatedTime ? new Date(deal.updatedTime).toLocaleDateString("ru-RU") : "—"}</span></div><b>{deal.opportunity ? money.format(deal.opportunity) : "→"}</b></button>)}</div>
